...
export async function forgotUserPassword(req, res) {
  try {
    ...
    if (!phone_number) return res.status(400).json({ success: false, error: 'Telefon numarası zorunludur.' });
    ...
    if (error.message.includes('USER_ERROR')) return res.status(404).json({ success: false, error: error.message.replace('USER_ERROR: ', '') });
...
export async function resetUserPassword(req, res) {
  try {
    ...
    if (!phone_number || !code || !new_password) return res.status(400).json({ success: false, error: 'Tüm alanlar zorunludur.' });
    ...
    if (error.message.includes('USER_ERROR')) return res.status(404).json({ success: false, error: error.message.replace('USER_ERROR: ', '') });
    if (error.message.includes('AUTH_ERROR')) return res.status(401).json({ success: false, error: error.message.replace('AUTH_ERROR: ', '') });
...
export async function changeUserPassword(req, res) {
  try {
    ...
    if (!current_password || !new_password || !password_confirm) return res.status(400).json({ success: false, message: 'Tüm alanlar zorunludur.' });
    if (new_password !== password_confirm) return res.status(400).json({ success: false, message: 'Şifreler eşleşmiyor.' });
...
export async function forgotAdminPassword(req, res) {
  try {
    ...
    if (!email) return res.status(400).json({ success: false, message: 'E-posta adresi zorunludur.' });
...
export async function setNewPassword(req, res) {
  try {
    ...
    if (!token || !new_password || !password_confirm) return res.status(400).json({ success: false, message: 'Tüm alanlar zorunludur.' });
    if (new_password !== password_confirm) return res.status(400).json({ success: false, message: 'Şifreler eşleşmiyor.' });
...
export async function changePasswordWithLogin(req, res) {
  try {
    ...
    if (!account) return res.status(401).json({ success: false, message: 'Yetkisiz işlem.' });
    ...
    if (!match) return res.status(400).json({ success: false, message: 'Mevcut şifre yanlış.' });
